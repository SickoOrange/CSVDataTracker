...
public class TableViewFactory {
  ...
  public static Function<Map<Integer, Module>, ObservableList<AfiTableViewModel>> collectAfiEntities() {
    return modules -> {
      ObservableList<AfiTableViewModel> entities = FXCollections.observableArrayList();
      modules.values().forEach(module -> entities
          .add(new AfiTableViewModel(module.getId(), module.getNode(), module.getAfiTypeId(),
  ...
  public static Function<List<Connection>, ObservableList<ConnectionTableViewModel>> collectConnectionEntities() {
    ...
    return connections -> connections.stream()
        .map(connection -> new ConnectionTableViewModel(connection.getOut().getAfiId(),
  ...
  public static Function<List<AfiType>, ObservableList<AfiTypeTableViewModel>> collectAfiTypeEntities() {
    return afiTypes -> afiTypes.stream()
        .map(afiType -> new AfiTypeTableViewModel(afiType.getTypeid(), afiType.getName(),
            afiType.getPort(),
